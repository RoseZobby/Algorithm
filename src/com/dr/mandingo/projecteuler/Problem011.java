...
public class Problem011 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < 20; i++) {
			for (int j = 0; j < 20; j++) {
				if (j + 3 < 20) {
					int rightValue = datas[i][j] * datas[i][j + 1]
							* datas[i][j + 2] * datas[i][j + 3];
					if (maxValue < rightValue) {
						maxValue = rightValue;
					}
				}
				if (i + 3 < 20) {
					int downValue = datas[i][j] * datas[i + 1][j]
							* datas[i + 2][j] * datas[i + 3][j];
					if (maxValue < downValue) {
						maxValue = downValue;
					}
				}
				if (i + 3 < 20 && j >= 3) {
					int diagonally1 = datas[i][j] * datas[i + 1][j - 1]
							* datas[i + 2][j - 2] * datas[i + 3][j - 3];
					if (maxValue < diagonally1) {
						maxValue = diagonally1;
					}

				}
				if (i + 3 < 20 && j + 3 < 20) {
					int diagonally2 = datas[i][j] * datas[i + 1][j + 1]
							* datas[i + 2][j + 2] * datas[i + 3][j + 3];
					if (maxValue < diagonally2) {
						maxValue = diagonally2;
					}
				}
			}
		}

		System.out.print("the final result is:" + maxValue);
	}
